...
public class BankGUI {
  ...
  */
  private void doAddAccount() {
    ...
    if (customerName == null) return; // Dialog was cancelled.
    // Request the address
    String customerAddress = JOptionPane.showInputDialog(
    "Enter Customer Address");
    if (customerAddress == null) return; // Dialog was cancelled.
    // Request the age
    String age = JOptionPane.showInputDialog("Enter Customer Age");
    if (age == null) return; // Dialog was cancelled.
    ...
    if (customerPhoneNumber == null) return; // Dialog was cancelled.
    ...
    if (choice == custType.length - 1) return; //Dialog was cancelled.
    ...
    commands, // List of commands
    commands[commands.length - 1]); // Default choice
    if (choice == commands.length - 1) return; //Dialog was cancelled.
  ...
  */
  private void doDeposit() {
    ...
    if (accountNumber == null) return; // Dialog was cancelled.
    
    String theAmount = JOptionPane.showInputDialog("Enter Amount");
    if (theAmount == null) return; // Dialog was cancelled.
